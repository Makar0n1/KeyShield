...
import { managerService, type AnonymizedChat } from '@/services/manager'
import { formatDate } from '@/utils/format'

type Tab = 'active' | 'closed'

export function ManagerDisputeChatsListPage() {
  const [tab, setTab] = useState<Tab>('active')
  const [chats, setChats] = useState<AnonymizedChat[]>([])
  ...
  const load = useCallback(async (t: Tab) => {
    ...
    setError(null)
    try {
      const { chats } = await managerService.listChats(t)
      setChats(chats)
    } catch (err) {
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <MessageSquare size={24} /> Чаты споров
        </h1>
        <p className="text-muted text-sm">
          Анонимизированный чат-арбитраж со сторонами. Личные данные сторон скрыты.
        </p>
      </div>
        ...
        {(['active', 'closed'] as Tab[]).map((t) => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
              tab === t ? 'border-primary text-white' : 'border-transparent text-muted hover:text-white'
            }`}
          >
      ...
      {loading ? (
        ...
        <Card className="p-12 text-center">
          ...
          {tab === 'active' && (
            <p className="text-xs text-muted mt-2">
              Чат открывается со страницы конкретного спора
            </p>
          )}
        </Card>
      ) : (
        <div className="space-y-2">
          {chats.map((c) => {
            const deal = c.deal && 'dealId' in c.deal ? c.deal : null
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs text-muted">{c.messages.length} сообщ.</span>
                      </div>
                      <div className="text-sm text-white truncate">
                        {(deal && 'productName' in deal && deal.productName) || 'без названия'}
                      </div>
                      <div className="text-xs text-muted mt-1">
                        {lastMsg
                          ? <>последнее: {formatDate(lastMsg.createdAt)}</>
                          : <>открыт: {formatDate(c.createdAt)}</>
                        }
                      </div>
                    </div>

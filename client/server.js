...
app.get('/api/admin/receipt/:dealId/:recipientType', adminAuth, async (req, res) => {
  try {
    const { dealId, recipientType } = req.params;

    if (!['buyer', 'seller'].includes(recipientType)) {
      return res.status(400).json({ error: 'recipientType must be "buyer" or "seller"' });
    }
    ...
    // Get user based on recipientType
    const telegramId = recipientType === 'buyer' ? deal.buyerId : deal.sellerId;
    const user = await User.findOne({ telegramId }).lean();
    if (!user) {
      return res.status(404).json({ error: `${recipientType} user not found` });
    }

    // Determine transaction type and data
    let transactionType, amount, txHash, toAddress;
    if (recipientType === 'buyer') {
      // Buyer gets purchase receipt
      transactionType = 'purchase';
      amount = deal.amount;
      txHash = deal.payoutTxHash || 'N/A';
      toAddress = deal.sellerAddress || 'N/A';
    } else {
      // Seller gets release receipt
      transactionType = 'release';
      amount = deal.amount - deal.commission;
      txHash = deal.payoutTxHash || 'N/A';
      toAddress = deal.sellerAddress || 'N/A';
    }
